...
function initWireframeDiagram() {
	url = window.location.href;
	var modelIndex = url.indexOf('.html#');
	...
	var lDialogId = "wireframeStatesDialog";
	
	currentStateImageId = lDefaultStateImageId;
	
	if (modelIndex != -1) {
		var modelId = url.substring(modelIndex + '.html#'.length);
		
		var lStateImageId = "diagram" + modelId;
		var lStateTitleText = document.getElementById("pageTitleText"+modelId).getAttribute("data-value");
		
		okWireframeStatesDialog(lDialogId, lStateImageId, lPageTitleId, lStateTitleText);
	}
	else {
		okWireframeStatesDialog(lDialogId, lDefaultStateImageId, lPageTitleId, lDefaultStateTitleText);
	}
...
function showWireframeStatesDialog(wireframeStatesDialogId, defaultStateImageId) {
	var wireframeStatesDialog = document.getElementById(wireframeStatesDialogId);
	...
	if (wireframeStatesDialog.style.display != "none") {
		...
		if (currentStateImageId == null) {
			currentStateImageId = defaultStateImageId;
		}
...
function okWireframeStatesDialog(wireframeStatesDialogId, stateImageId, pageTitleId, stateTitleText) {
	var wireframeStatesDialog = document.getElementById(wireframeStatesDialogId);
	wireframeStatesDialog.style.display = "none";
...
function showFoeWireframeStatesDialog(stateDialogId, selectStateDialogButtonId) {
	...
	if (stateDialog.style.display != "none") {
		...
		if (currentSelectStateButton != null) {
			currentSelectStateButton.style.marginRight = 0;
		}
